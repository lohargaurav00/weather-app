...
const Sunset: React.FC<SunsetProps> = ({ sunrise, sunset, timezone }) => {
  return (
    <Card className="flex h-52 sm:h-48 flex-col justify-between overflow-hidden">
      <CardHeader>
        <CardTitle className="inline-flex items-center justify-start gap-2 font-semibold text-lg">
          <i>
            <svg
              viewBox="0 0 24 24"
              fill="none"
      ...
      <CardContent>
        <p>{formatSunTimeWithAMPM(sunset, timezone)}</p>
      </CardContent>
      <CardFooter>
        <p>Sunrise: {formatSunTimeWithAMPM(sunrise, timezone)}</p>
      </CardFooter>
    </Card>
  );
};
